    ...
    def insert_articles_by_keyword(self, keyword: str, created_by: str, epmc_db: str) -> dict[str, int]:
        ...
        try:
            for article in results: 
                ...
                # Article
                article_entity = self.epmc_client.create_article(article, article_record_id, created_by=created_by)
                ...
                for grant_data in (article.get("grantsList") or {}).get("grant") or []:
                    ...
                    grant_entity = self.epmc_client.create_grant(grant_data, article_record_id, created_by=created_by)
                ...
                #    citation_entity = self.epmc_client.create_citation(cite, article_id, created_by=created_by)
                #    citation_id = self.epmc_repo.insert_or_update(citation_entity, Citation, existing_citation)
                #    counts["citations"] += 1

                # References
                # response = self.epmc_client.get_references(article.get("id"))
                ...
                #     reference_entity = self.epmc_client.create_reference(ref, article_id, created_by=created_by)
                ...
                for ft in (article.get("fullTextUrlList") or {}).get("fullTextUrl") or []:
                    ...
                    fulltext_entity = self.epmc_client.create_fulltext(article_id, ft, created_by=created_by)
                ...
                for author in (article.get("authorList") or {}).get("author") or []:
                    ...
                    if existing is None:
                        author_entity = self.epmc_client.create_author(author, created_by=created_by)
                    ...
                    article_author_entity = self.epmc_client.create_article_author(article_id, author_id, author_order, created_by=created_by)
                    ...
                    for aff in (author.get("authorAffiliationDetailsList") or {}).get("authorAffiliation", []) or []:
                        ...
                        if org_name and org_name not in affiliations_seen:
                            ...
                            affiliation_entity = self.epmc_client.create_affiliation(aff, author_id, article_id, aff_order, created_by=created_by)
